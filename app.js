...
const modelApiMap = { 'Default':'GPT-5-mini', 'KmR si':'GPT-5-mini', 'oM 5i':'GPT-5-mini', 'GPT-5-mini':'GPT-5-mini', 'GPT-5-pro':'GPT-5-pro' };
let currentUtterance = null;
let activeMode = localStorage.getItem('omenis_active_mode') || null;
...
function createMessageElement(content, role, isGenerating = false) {
  ...
  if (role === 'assistant') {
    ...
    speakBtn.innerHTML=`<svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor"><path d="M11 5l-6 4H2v6h3l6 4V5z"/><path d="M19 7c1.5 1.5 1.5 8.5 0 10"/></svg>`;
    // Toggle speak: if currently speaking, cancel; otherwise start speaking.
    speakBtn.addEventListener('click',()=> {
      const text = typewriterDiv.textContent.trim();
      if (!text) return;
    ...
    copyButtonContainer.appendChild(speakBtn);
...
function createCopyButton(getTextCallback) {
  ...
  button.addEventListener('click', async () => {
    ...
    try {
      ...
      button.innerHTML = `
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" width="14" height="14">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l-6 6L19 7" />
        </svg>
      `;
...
async function sendToAPI(message) {
  try {
    ...
    // Build Gemini-style payload with conversation contents
    ...
    // Use XMLHttpRequest as requested, with provided RapidAPI details
    const responseText = await new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.withCredentials = true;
      xhr.open('POST', 'https://gemini-pro-ai-new.p.rapidapi.com/chat?key=AIzaSyDZt1mpiniiBIWYbXjzSujIvmigUOkqU6E');
      xhr.setRequestHeader('x-rapidapi-key', '9c8a6d3509msh57773c267fd8d26p16eb47jsn2361fd22e3bd');
      xhr.setRequestHeader('x-rapidapi-host', 'gemini-pro-ai-new.p.rapidapi.com');
      xhr.setRequestHeader('Content-Type', 'application/json');
      xhr.timeout = 60000;
      xhr.onreadystatechange = function () {
        if (this.readyState === this.DONE) {
          if (this.status >= 200 && this.status < 300) resolve(this.responseText);
          else reject(new Error(`HTTP ${this.status}: ${this.responseText}`));
        }
      };
    ...
    // Parse response and extract text (support several possible shapes)
    let parsed;
    try { parsed = JSON.parse(responseText); } catch (e) { parsed = null; }
    let aiContent = '';

    if (parsed) {
      // Try common nested locations used by Gemini-style responses
      if (parsed?.candidates?.[0]?.content?.parts?.[0]?.text) {
        aiContent = parsed.candidates[0].content.parts[0].text;
      } else if (parsed?.response?.candidates?.[0]?.content?.parts?.[0]?.text) {
        aiContent = parsed.response.candidates[0].content.parts[0].text;
      } else if (typeof parsed?.reply === 'string') {
        aiContent = parsed.reply;
      } else if (parsed?.message) {
        aiContent = parsed.message;
      } else {
        // Fallback: find first long string value
        for (const k in parsed) {
          if (typeof parsed[k] === 'string' && parsed[k].length > 20) { aiContent = parsed[k]; break; }
        }
      }
    }

    if (!aiContent || !aiContent.trim()) {
      aiContent = generateMeaningfulResponse(message);
    }
...
function initializeNewChat() {
  ...
  if (!newChatButton || !menu) return;
  
  const syncActive = () => {
    menu.querySelectorAll('.new-option').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.action === activeMode);
    });
  };
  ...
  newChatButton.addEventListener('click', toggle);
  document.addEventListener('click', (e) => {
    if (!menu.contains(e.target) && e.target !== newChatButton) hide();
  });
  ...
  menu.querySelectorAll('.new-option').forEach(btn => {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (action === 'new') {
        createNewChat();
        activeMode = null;
        localStorage.removeItem('omenis_active_mode');
      } else {
        activeMode = action;
        localStorage.setItem('omenis_active_mode', activeMode);
      }
      syncActive();
      hide();
    });
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  initializePrivacy(); // NEW: privacy overlay
  ...
  const ov=document.getElementById('welcomeOverlay');const btn=document.getElementById('welcomeContinue');if(ov&&btn){ov.style.display='flex';btn.addEventListener('click',()=>ov.style.display='none');}
});
...
function initThemeToggle(){
  ...
  const states=['off','dark','rgb'];
  function apply(state){
    const root=document.documentElement;
    if(state==='off'){ delete root.dataset.theme; btn.title='Theme: Off'; btn.querySelector('.label').textContent='Off'; btn.querySelector('.dot').style.background='var(--text-secondary)'; }
    if(state==='dark'){ root.dataset.theme='dark'; btn.title='Theme: Dark'; btn.querySelector('.label').textContent='Dark'; btn.querySelector('.dot').style.background='#111827'; }
    if(state==='rgb'){ root.dataset.theme='rgb'; btn.title='Theme: RGB'; btn.querySelector('.label').textContent='RGB'; }
    localStorage.setItem('omenis_theme', state);
  }
  let current=localStorage.getItem('omenis_theme')||'off'; apply(current);
  btn.addEventListener('click',()=>{ const i=(states.indexOf(current)+1)%states.length; current=states[i]; apply(current); });
}

function initializePrivacy(){
  const link=document.getElementById('privacyLink'),ov=document.getElementById('privacyOverlay');
  const close=document.getElementById('privacyClose'),back=document.getElementById('privacyBack');
  if(!link||!ov||!close||!back) return;
  const hide=()=>ov.style.display='none';
  link.addEventListener('click',()=>ov.style.display='block');
  close.addEventListener('click',hide); back.addEventListener('click',hide);
}

function speakText(text, btn){
  try{
    // If already speaking, cancel and toggle off
    if(window.speechSynthesis && window.speechSynthesis.speaking){
      window.speechSynthesis.cancel();
  ...
  }catch(e){ console.error('TTS error:', e); btn?.classList.remove('speaking'); }
}